...
final class StackTraceMapper {
    ...
    String mapStackText(String stack) {
    ...
    private String mapStackLine(String line) {
        ...
        while (matcher.find()) {
            ...
            String displayPath = virtualModules.displayPath(path);
            if (displayPath == null) {
                displayPath = path;
            }
            int lineNumber = Integer.parseInt(matcher.group(3));
            int columnNumber = matcher.group(4) == null ? 1 : Integer.parseInt(matcher.group(4));
            SourceMapRegistry.OriginalPosition position = sourceMaps.getMappedPosition(displayPath, lineNumber, columnNumber);
