...
public final class NekoEsmParser {
    ...
    public NekoEsmModuleAst parse() {
        ...
        scopeStack.push(new ScopeFrame(0, -1, NekoEsmScopeKind.MODULE, 0));
        int i = 0;
        while (i < length) {
    ...
    private void recordLocalDeclaration(int start, String word) {
        ...
        if (("const".equals(word) || "let".equals(word) || "var".equals(word)) && (first == '{' || first == '[')) {
            ...
            if (declarationEnd > declarationStart) {
                recordPatternBindings(code.substring(declarationStart, declarationEnd), word, new NekoEsmSpan(declarationStart, declarationEnd), declarationScopeId(word));
            }
        }
    }
    ...
    private int declarationScopeId(String kind) {
        ...
        if ("var".equals(kind)) {
            return nearestFunctionOrModuleScopeId();
        }
    ...
    private int nearestFunctionOrModuleScopeId() {
        for (ScopeFrame frame : scopeStack) {
            if (frame.kind == NekoEsmScopeKind.FUNCTION || frame.kind == NekoEsmScopeKind.MODULE) {
                return frame.id;
            }
        }
        return 0;
    ...
    private void openBraceScope(int start) {
        NekoEsmScopeKind kind = functionBodyStarts.remove(start) ? NekoEsmScopeKind.FUNCTION : NekoEsmScopeKind.BLOCK;
        int id = nextScopeId++;
        scopeStack.push(new ScopeFrame(id, currentScopeId(), kind, start));
    }
    ...
    private void closeScope(int end) {
        ...
        scopes.add(new NekoEsmScope(frame.id, frame.parentId, frame.kind, new NekoEsmSpan(frame.start, end)));
    }
    ...
    private record ScopeFrame(int id, int parentId, NekoEsmScopeKind kind, int start) {}
}
