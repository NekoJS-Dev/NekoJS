...
`config/engine.toml` 的 `scriptRunawayTimeoutSeconds = 2` 确定性失败注入）→ 启动
...
- **失败注入**：`while(true){}`（runner 写入 `server_scripts/sm02-boom.js`）由 runaway
  watchdog 在 ~2s 后关闭候选 Context → reload 以 `phase=EXECUTION` /
  `domain=candidate-killed` / `source=server/sm02-boom.js` 失败，active 保持分发。
  若 engine.toml 未设 watchdog，`while(true)` 会永久阻塞服务器线程（脚本会硬失败以保证不静默）。
