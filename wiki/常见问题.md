...
A: 确认你的文件被当 ESM 处理：
- 用 `.mjs` 扩展名最稳妥。
- 或确认 `config/nekojs-engine.toml` 里 `enableEsmAuthoring = true`（默认开）。
...
A: 在 `config/nekojs-engine.toml`（游戏根 config 目录；旧 `nekojs/config/engine.toml` 仅作只读回退）设置 `jsxAutomaticRuntime = true`，并在 `nekojs/` 工作区内将 `jsx-runtime.js` 放在裸模块解析路径 `node_modules/nekojs/jsx-runtime.js`。它必须导出标准的 `jsx`、`jsxs` 和 `Fragment`：编译结果将子节点传入 `props.children`，单/零子节点使用 `jsx`，多子节点使用 `jsxs`，片段把 `Fragment` 作为 type。
...
A: 沙盒限制——只能访问游戏目录内。详见 [Node.js 兼容 - 访问范围限制](Node-js-兼容#访问范围限制)。如需放宽（仅单人/可信环境），看 `config/nekojs-engine.toml`。
