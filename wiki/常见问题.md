...
- **配方（NeoForge）**：NeoForge 上 `/nekojs reload` 重跑脚本但配方已固化，需重新进世界。详见 [命令 - 配方 reload](命令#配方-reload-的平台差异)。
...
A:
- **NeoForge**：配方在数据包加载阶段固化。`/nekojs reload` 会重跑脚本但配方面板不刷新——**重新进世界**。详见 [命令](命令)。
...
A: 脚本应视为**可信代码**。沙盒限制了 Java 类访问和文件范围，但如果你开了远程脚本同步（NetworkEvents + `ScriptSyncService`），客户端能同步脚本到服务器，需要你信任连接的玩家。详见 `ai_arch/nekojs-architecture.md` 的网络同步章节。
