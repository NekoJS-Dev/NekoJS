...
A: 在 `nekojs/config/engine.toml`（与 probe.toml 同目录；旧 `config/nekojs-engine.toml` 仅作只读回退）设置 `jsxAutomaticRuntime = true`，并在 `nekojs/` 工作区内将 `jsx-runtime.js` 放在裸模块解析路径 `node_modules/nekojs/jsx-runtime.js`。它必须导出标准的 `jsx`、`jsxs` 和 `Fragment`：编译结果将子节点传入 `props.children`，单/零子节点使用 `jsx`，多子节点使用 `jsxs`，片段把 `Fragment` 作为 type。
