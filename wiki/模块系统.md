...
- **配置**：`nekojs/config/engine.toml`（与 probe.toml 同目录；旧 `config/nekojs-engine.toml` 仅作只读回退）可调 `allowThreads`、`allowReflection`、`allowAsm`、`enableEsmAuthoring`、`scriptStatementLimit`（默认 50,000,000 宽松上限，0 禁用）等。
