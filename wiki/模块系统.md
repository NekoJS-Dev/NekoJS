...
- **失控保护**：`scriptRunawayTimeoutSeconds`（默认 10，0 禁用）——同步脚本持续执行超过该秒数未让出（判定为失控循环，如 `while(true){}`）即中止并自动重建脚本环境；每次让出（事件间隙/长宿主调用）都会重新计时，长驻环境无论累计执行多少语句都不会被误杀。另有可选的 `scriptStatementLimit`（**默认 0 禁用**）：单个脚本源连续执行过的语句总量硬上限，仅在需要硬性预算时启用。
