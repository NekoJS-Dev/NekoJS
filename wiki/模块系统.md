...
- ⚠️ **只支持纯 JS 包**。包含原生 bindings（C/C++ 编译产物）的包**不能用**（如 `node-sass`、`sharp`、`better-sqlite3`）。
...
- **Java 类**：高危类被过滤（`java.lang.Runtime`、`Process`、`ClassLoader`、`System`、`java.io.*`、`java.nio.*`、反射、net、lwjgl、polyglot 本身等）。具体黑名单见 `common/.../core/fs/ClassFilter.java`。
- **文件系统**：访问范围限制在游戏目录内，符号链接逃逸检查。
- **配置**：`nekojs/config/engine.toml` 可调 `allowThreads`、`allowReflection`、`allowAsm`、`enableEsmAuthoring` 等。

> 脚本仍应视为**可信代码**，尤其是在多人服务器里用网络同步功能时。

## 顶层 await 与动态 import

```javascript
// 顶层 await：脚本加载阶段就能等异步操作
const data = await JsonIO.read('./config.json')
console.info(data)
