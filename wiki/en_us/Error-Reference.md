...
problem carry a stable code and a short English summary, so a log line can be
understood, and looked up here, without reading Chinese:
...
| Code | English summary | Chinese message |
|---|---|---|
| `NEKO-2001` | script context hit resource limits | 脚本环境 {} 触发 ResourceLimits（失控看门狗 {}s / 语句上限 {}），Graal 已关闭该 Context；当前求值被中止，下一次取用时会自动重建（/n… |
| `NEKO-2002` | script statement limit exceeded | 脚本语句累计数达到 scriptStatementLimit（{}），关闭对应脚本环境；当前求值被中止，下一次取用时会自动重建 Context（/nekojs reload 亦… |
| `NEKO-2003` | runaway script loop detected | 脚本同步执行持续超过 scriptRunawayTimeoutSeconds（{}s）未让出，判定为失控循环，关闭对应脚本环境；当前求值被中止，下一次取用时会自动重建 Cont… |
| `NEKO-2004` | script output line limit reached | 脚本输出行数超过 {} 行上限，后续输出将被丢弃（防止日志无限增长） |
| `NEKO-2005` | script evaluation timed out | 脚本求值超时（超过  秒，可在 nekojs/config/engine.toml 中调整 scriptEvaluationTimeoutSeconds）：入口脚本的顶层 aw… |
...
Routine progress messages carry an English summary but no code, since there
is nothing to look up when they appear.
