...
| `Ingredient.not(ingredient)` | ⚠️ **不支持**，会抛错。用 `base.except(ingredient)` 代替 |
...
JSON 读写。GraalJS 会自动把 JS 对象 ↔ `Map`/`List` 互转。
...
| 方法 | 侧 | 说明 |
|---|---|---|
| `Network.sendToServer(channel, data)` | client | 客户端 → 服务端 |
| `Network.sendToPlayer(player, channel, data)` | server | 服务端 → 单个玩家 |
| `Network.sendToAll(channel, data)` | server | 服务端 → 所有玩家 |
