...
### `NativeEvents`（仅 STARTUP，已弃用）

> **已弃用**：`NativeEvents` 已标记 `@Deprecated`，请迁移到 [`ScriptEvents`](#scriptevents仅-startup)（声明式组名注册，能力对等：priority / receiveCancelled / 返回 `true` 取消 / 按脚本 reload 清理监听器）。将在弃用窗口结束后移除；Cleanroom 1.12.2 上本就不可用（抛异常）。
