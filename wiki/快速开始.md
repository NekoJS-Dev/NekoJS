...
NekoJS 依赖 [Graal](https://www.curseforge.com/minecraft/mc-mods/graal)（提供 GraalJS 运行时），需要 **25.1.3.7 或更高**——更低的版本没注册正则语言，脚本里一用正则就会抛 `No language for id regex found`。
...
- 1.12.2 没有配置阶段，只能退化成登录后再同步，而且只同步纯脚本。
