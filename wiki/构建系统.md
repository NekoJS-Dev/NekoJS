...
`.github/workflows/ci-build.yml`（手写节点矩阵）：`:common:check` → `guardLint` → 各节点 `nbtSmokeTest` → `npm run test:probe-types` → 各节点 `build`。发版由提交标题 `update <版本>` 触发。注意仓库 `gradle.properties` 钉了本机 `org.gradle.java.home`，CI 里用 `sed` 删除。
