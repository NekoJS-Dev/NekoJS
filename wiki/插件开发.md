...
> **顺序契约**：同优先级插件的相对顺序是发现/扫描序，跨平台不稳定——不要依赖同优先级插件间的注册顺序。需要确定性地覆盖内置实现时用各注册表的显式 API（如 `ScriptCompilerRegistry.replaceLanguage`）；编译器按扩展名查找是「后注册者胜」，即优先级**低于**内置的插件可用普通 `register` 覆盖内置语言。同一插件类被重复发现（classpath 重复条目）会自动去重，只注册一次。

## NekoJSPlugin 钩子一览

> **双形态模型（ADR-0010）**：每条通道的**事实源**是自包含扩展点文件（`core.plugin` 包的 `XxxPoint`：冲突策略、产物冻结、依赖关系全在点内）；`NekoJSPlugin` 的钩子是各点的**门面投影**。覆写基接口钩子（推荐、最简）与 implements `XxxPoint.Contributor`（显式形态）由同一个点收集，**效果完全等价**——本页统一按基接口钩子描述。`PluginHookPairingTest` 守护两者配对：在基接口加收集型方法必须配对 Point，否则 CI 红。
...
第三方也可以定义自己的收集型扩展点——走与内置完全相同的 V2 builder 形态（完整语义见 ADR-0001/0002）：
