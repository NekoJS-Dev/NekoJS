...
| 钩子 | 作用 |
|---|---|
| `registerProbeBackends(ProbeBackendRegistry)` | 注册 probe backend（按 `(languageId, name)` 二维登记；内置 `("typescript", "builtin")`，第三方可加 `("python", "builtin")` 产出 `.pyi` 等，同 `(语言, 名字)` 冲突在 bootstrap 时报错） |
