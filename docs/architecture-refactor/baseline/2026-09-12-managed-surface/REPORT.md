...
| AC4 能力三态 + loader/version/context 条件 + 显式失败 | **pass**（review 后条件成为权威 gate） | `CapabilityStatus` 三态 + `ContractCapability.status/conditions` + `CapabilityDefinition.declaredStatus` + resolver 接线（`ManagedSurfaceCapabilityConditionsTest` 7 用例：条件匹配激活、loader 越界显式 UNAVAILABLE、PARTIAL 可观察、声明 UNAVAILABLE 即使有 provider 也不激活（DECLARED_UNAVAILABLE）、loader 版本门控、manifest 只暴露 active 结论、兼容构造）。保持既有 capability 模型，**未新增通用框架**（AC10）。**review 修订**：发现 v1 的 isEligible 只看 provider scope（契约条件未权威生效、负样本缺失）→ 已修（契约 environmentScope 为权威 gate，provider 无 scope/更宽 scope 都不能替契约放宽；两条负样本用例 + 更宽 provider 的 SCOPE_NOT_CONTAINED fail-fast 用例）。注记：真实契约当前 capabilities=空集（涉及能力的符号由功能域票 14/15/16 陆续注册），机制由 fixture 覆盖；TS/Py 生成器暂不渲染 capability status/conditions（渲染属后续 W5 跟进） |
...
| AC6 Builder 显式 setter = property 写入语义 + 代表性 fixture | **pass（本票口径）** | `ManagedBuilderWriteSemanticsTest`（4 用例）：显式 setter 与类型化 put 同一规范化路径、build() final identity、只读成员只承诺访问器、builder 创建的契约/manifest/TS 派生 parity；Graal 天然 Bean 行为不作为承诺（不做契约，javadoc+MIGRATION 注明）。启动期/动态/修改域覆盖归各域票（工单 AC6 明示不反向 blocker）。**review 补充 runtime fixture（真实 Graal 执行）**：现役受管 Builder 形状是 public 字段（`ItemBuilder.maxStackSize`），脚本端 `b.maxStackSize = 16` = 宿主字段写——fixture 钉死该路径（Graal 升级改变行为会先红）。实测发现：private 字段+裸 bean setter 形状的 property 写**静默不生效**（不抛错不触 setter）——"Graal 天然 Bean 行为不作为承诺"的实证；setter 等价分发由受管 Builder 机制（票 15/39，ADR-0005 修订）实现。compound 值经契约 invoker 的 NATIVE_TYPE_LEAK 见 §7.2（owner W4/W5 跟进） |
...
| AC11 至少一个真实脚本穿透完整链路 | **pass** | `ManagedSurfaceEndToEndChainTest`：同一脚本（ID/Text/NBT 调用，Graal 运行时经 ApiFacadeProxy 契约 invoker）→ 段 1 契约反射断言 → 段 2 manifest 断言 → 段 3 TS declaration 断言 → 段 4 Python IR 反射断言（fixture 反射对象，非脚本符号——Python 生成器今天消费 catalog IR 而非 managedApis，统一归 W5 跟进；Python 侧符号 parity 由 AC5 的 parity fixture 单独承载） → 段 5 运行时结果 `{id:"nekojs:chain",label:"chain-example",count:3}`。`npm run test:probe-types` 通过（TS 契约校验）。外部 addon/Probe 差异按工单属协调项，不在本票冒充验收 |
