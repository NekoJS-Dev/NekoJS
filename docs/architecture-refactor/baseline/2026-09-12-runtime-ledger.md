...
| A2 | `NekoJSFabricMod.RUNTIME_ROOT` | `versions/26.1.2-fabric/src/main/java/com/tkisor/nekojs/fabric/NekoJSFabricMod.java:59`（赋值 :154；ticket 05 后改为 private + `volatile`，同包经 `runtimeRootOrNull()` 访问） | 待删除 legacy bypass（public static root）→ 已删除并收敛为 package-private accessor seam | 同上 | 同上 | 删除条件：fabric entrypoint 由 loader 反射实例化、无法构造注入，accessor 是最小 seam；**当 W7/W8 提供可注入的 fabric composition 通道时删除 `runtimeRootOrNull()` 并改构造注入**。风险注记：3 个 `bind(root)` 监听器（PDataSyncListener/ServerEventListener/PackSyncClientConnections）无 unbind——生产进程 root 常驻时无害，06/07 引入 root 重建后必须补 unbind，否则静默持旧 handle |
