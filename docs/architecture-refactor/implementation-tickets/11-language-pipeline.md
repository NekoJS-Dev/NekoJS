...
| AC10 | `ModulePipelineIsolationTest` 源码扫描/纯签名扫描/无零参或隐式 default helper 构造器/绝对路径 source-map helper；host 只接收 resolver/cache；平台依赖只在 execution assembly | `./gradlew.bat :common:test --tests com.tkisor.nekojs.core.module.ModulePipelineIsolationTest`（并由 `:common:check` 重跑） | PASS；反射与去注释扫描覆盖 `NekoModulePipelineCache`、`NekoModuleResolver`、loader host、source-map/virtual registry、error tracker，禁止 `NekoJSPaths.get()`、`Platform.getGameDir` 与 `defaultPreparationCache`；common compile 不创建 Context。 |
