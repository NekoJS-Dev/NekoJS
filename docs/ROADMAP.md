...
- [ ] **Katton 能力参考采纳**（2026-08-20 计划成文，详见 `ai_arch/katton-adoption-plan.md`）：P0 所有权/作用域模型 → P1 脚本包形态（manifest + 全局/世界双作用域）→ P2 多人脚本包分发（配置阶段推送 + Ed25519 签名 + 信任存储）→ P3 运行时注册（冻结绕过 + stale-retain + 退世界注销，CR 1.12.2 不做）→ P4 内容修改/村民交易/advancement 合并/包资源挂载 → P5 客户端渲染（HUD/post-effect/虚拟物品 marker）+ QoL（reload 进度/EntitySelectorBuilder/once）。全部默认关闭、逐阶段 opt-in；ByteBuddy 注入等不采纳项及理由见计划 §11。
