...
- [ ] 为 Node-compatible API 补 catalog manual declarations，让外部类型工具可为 `require('fs')` / `require('node:fs')` 等生成准确类型。
...
- [x] 重构脚本编译接口：新增 `IScriptCompiler.compileDetailed(...) -> ScriptCompileResult(code, sourceMap)`，让语言插件/legacy 编译器能返回 sourcemap；旧 `compile(...) -> String` 暂作为兼容默认入口保留。
