...
/**
 * 26.2.0-fabric 的 {@link McVersionCompat} 接线回归：本节点 jar/test classpath 必须携带
 * {@code Fabric262VersionCompat} services 条目——共享树 {@code inject.MixinLevel} 注入的
...
class FabricVersionCompatWiringTest {
    ...
    void fabricNodeProvidesVersionCompatImpl() {
        assertTrue(ServiceLoader.load(McVersionCompat.Impl.class, McVersionCompat.class.getClassLoader())
                        .findFirst().isPresent(),
                "no McVersionCompat.Impl provider on 26.2.0-fabric test classpath: "
                        + "expected META-INF/services entry for Fabric262VersionCompat");
    }
}
