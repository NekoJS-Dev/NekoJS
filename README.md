...
* **TypeScript & JSX 扩展能力**：NekoJS 本体内置 `.ts` erasable TypeScript 前端和轻量 `.jsx/.tsx` classic runtime lowering；enum、namespace、decorator、更完整 TSX 等高级语法可通过 NekoSWC 或其他注册到 NekoJS 的脚本语言插件扩展。
...
当前可执行脚本目录为 `startup_scripts/`、`server_scripts/` 和 `client_scripts/`。脚本文件支持 `.js`、`.mjs`、`.cjs`、内置 erasable `.ts`，以及轻量 `.jsx/.tsx` classic runtime lowering；更复杂的 TS/TSX 语法需要可用的编译器扩展（推荐 NekoSWC）。
...
NekoJS 核心主打轻量与稳定，内置 `.ts` 的 erasable TypeScript 支持：类型标注、`type` / `interface`、`import type` / `export type` 等会在 Java 前端中擦除，之后继续走 NekoJS 自有 ESM/CJS pipeline。NekoJS 也内置轻量 `.jsx/.tsx` classic runtime lowering，会把 JSX 元素降到 `globalThis.__nekoJsxFactory(...)` / `globalThis.__nekoJsxFragment(...)`。需要 enum、namespace、decorator、自动 JSX runtime 或更完整 TypeScript/TSX 转换时，请安装官方附属模组 **[NekoSWC](https://github.com/Tki-sor/NekoSWC)** 或其他脚本语言插件。
