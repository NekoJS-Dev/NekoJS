...
* **TypeScript & JSX 扩展能力**：NekoJS 本体内置 `.ts` erasable TypeScript 前端，复杂 TS/JSX/TSX 可通过 NekoSWC 或其他注册到 NekoJS 的脚本语言插件扩展。
...
当前可执行脚本目录为 `startup_scripts/`、`server_scripts/` 和 `client_scripts/`。脚本文件支持 `.js`、`.mjs`、`.cjs` 和内置 erasable `.ts`；`.jsx`、`.tsx` 以及更复杂的 TS 语法需要可用的编译器扩展（推荐 NekoSWC）。
...
NekoJS 核心主打轻量与稳定，内置 `.ts` 的 erasable TypeScript 支持：类型标注、`type` / `interface`、`import type` / `export type` 等会在 Java 前端中擦除，之后继续走 NekoJS 自有 ESM/CJS pipeline。需要 enum、namespace、decorator、JSX/TSX 或更完整 TypeScript 转换时，请安装官方附属模组 **[NekoSWC](https://github.com/Tki-sor/NekoSWC)** 或其他脚本语言插件。

NekoSWC 这类插件应注册到 NekoJS 的脚本语言前端/编译器 registry，负责高级 TS/TSX/JSX 语法转 JS，并可返回 sourcemap；NekoJS 本体继续负责模块解析、native ESM runtime、diagnostics 和执行。
...
这个流程的生命周期是固定的：先扫描并实例化所有 `@RegisterNekoJSPlugin`，再收集所有 `NekoPluginExtensionProvider` 注册的插件类型，冻结 extension point registry 后才执行各插件的 typed hooks。extension point 的 collector 只能访问受限的 `NekoPluginExtensionContext` registry，不会拿到 `NekoPluginRuntime` 内部集合。
