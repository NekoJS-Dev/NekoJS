...
/**
 * 1.12.2 RecipeFilter adapter：把 JS 过滤条件解析为结构化 {@link RecipeFilter}。
 *
 * <p>与 datapack 时代平台的 {@code RecipeFilterAdapter} 语义对齐：顶层数组为
 * {@link RecipeFilter.Or}，扁平对象为 {@link RecipeFilter.And}（{@code not} 包
 * {@link RecipeFilter.Not}，其余键作为 And 条件）。目标类型收窄为专用
 * {@link RecipeFilter}（而非 {@code Object.class}），避免经 HostAccess 的
 * {@code targetTypeMapping(Value.class, Object.class, ...)} 劫持任意 Object 形参。
 *
...
public final class RecipeFilterAdapter implements JSTypeAdapter<RecipeFilter> {
    ...
    @Override
    public Class<RecipeFilter> getTargetClass() {
        return RecipeFilter.class;
    }
