...
ServerEvents.started(event => {
    ...
    // B1: SoundEvent（callback builder）
    const boom = DynamicRegistry.soundEvent('nekojs_smoke:boom', builder => builder
        .mode('world').fixedRange(16.0))
    console.log(`soundEvent handle: ${boom.id()} mode=${boom.mode()} owner=${boom.owner()}`)

    // B1: MobEffect（callback builder）
    const witherTouch = DynamicRegistry.mobEffect('nekojs_smoke:wither_touch', builder => builder
        .mode('world').category('harmful').color(0x8B0000))
    console.log(`mobEffect handle: ${witherTouch.id()} mode=${witherTouch.mode()}`)

    // B2: Item（26.x 组件路径：注册后立即绑定 holder 默认组件）
    const ruby = DynamicRegistry.item('nekojs_smoke:ruby', builder => builder
        .mode('world').maxStackSize(64).rarity('epic').fireResistant())
    console.log(`item handle: ${ruby.id()} mode=${ruby.mode()}`)

    // 幂等复跑：同 id 二次注册 → 复用已注册条目并 re-claim（不抛错、不重复）
    const rubyAgain = DynamicRegistry.item('nekojs_smoke:ruby', builder => builder
        .mode('world').maxStackSize(64))
    console.log(`item re-claim: ${rubyAgain.id()} (same entry reused)`)

    // RELOADABLE 模式：v1 行为与 WORLD 相同（仅记账差异）
    const ping = DynamicRegistry.soundEvent('nekojs_smoke:ping', builder => builder.mode('reloadable'))
