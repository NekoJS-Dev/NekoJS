...
public final class FluidResolver {
    ...
    public static FluidIngredient combine(List<FluidIngredient> alternatives) {
        ...
        for (FluidIngredient ingredient : alternatives) {
            holders.addAll(ingredient.fluids());
        }
        if (holders.isEmpty()) throw new IllegalArgumentException("FluidIngredient cannot be empty");
        return FluidIngredient.of(HolderSet.direct(holders));
    }
    ...
    public static Fluid getFluid(String raw) {
        ...
        if (fluid == Fluids.EMPTY && !id.getPath().equals("empty")) throw new IllegalArgumentException("Fluid not found: " + id);
        return fluid;
    }
