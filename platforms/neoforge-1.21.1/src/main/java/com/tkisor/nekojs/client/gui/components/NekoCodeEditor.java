...
public class NekoCodeEditor {
    ...
    public NekoCodeEditor(Font font, int x, int y, int width, int height, String initialText, Runnable onSave) {
        ...
        // 在 1.21.1 中初始化 MultiLineEditBox
        this.editorBox = new MultiLineEditBox(font, x + GUTTER_WIDTH + 2, y + 4, width - GUTTER_WIDTH - 6, height - 8, Component.empty(), Component.empty());
        ...
        // 获取内部的 textField
        MultilineTextField textField = getTextFieldFromBox();

        if (textField != null) {
            textField.setCursorListener(() -> {
                if (!isRestoringHistory) {
                    lastCursor = textField.cursor();
                    lastScroll = this.getScrollAmountSafely();
                }
        ...
        this.setScrollAmountSafely(0);
        this.setCursorAbsolute(0);
    }

    // ==========================================
    // 安全获取受保护字段的方法 (通过反射临时解决)
    ...
    private int getCursorSafely() {
        MultilineTextField tf = getTextFieldFromBox();
        return tf != null ? tf.cursor() : 0;
    }

    private int getSelectCursorSafely() {
        MultilineTextField tf = getTextFieldFromBox();
        return tf != null ? tf.selectCursor : 0;
    }

    private void insertTextSafely(String text) {
        MultilineTextField tf = getTextFieldFromBox();
        if (tf != null) {
            tf.insertText(text);
        }
    ...
    private int getLineAtCursorSafely() {
        MultilineTextField tf = getTextFieldFromBox();
        return tf != null ? tf.getLineAtCursor() : -1;
    }

    private MultilineTextField.StringView getLineViewSafely(int line) {
        MultilineTextField tf = getTextFieldFromBox();
        return tf != null ? tf.getLineView(line) : null;
    }
    // ==========================================
    ...
    public void scrollToCursorCentered() {
        int cursorLine = getLineAtCursorSafely();
    ...
    public void setSelection(int cursor, int selectCursor) {
        ...
        MultilineTextField tf = getTextFieldFromBox();
        if (tf != null) {
            tf.cursor = Math.max(0, Math.min(cursor, len));
            tf.selectCursor = Math.max(0, Math.min(selectCursor, len));
        }
    }
    ...
    public boolean charTyped(char c) {
        ...
        if (pair != null) {
            ...
            int cursor = getCursorSafely();
            int selectCursor = getSelectCursorSafely();
        ...
        if (isClosingBracket(c)) {
            String text = editorBox.getValue();
            int cursor = getCursorSafely();
            if (cursor < text.length() && text.charAt(cursor) == c && cursor == getSelectCursorSafely()) {
                setCursorAbsolute(cursor + 1);
                return true;
            }
        }
    ...
    // 1.21.1 修复：将 KeyEvent 替换为底层的 keyCode, scanCode, modifiers
    ...
    private void pushCurrentState() {
        if (isRestoringHistory) return;
        undoStack.push(new HistoryState(editorBox.getValue(), getCursorSafely(), getScrollAmountSafely()));
        redoStack.clear();
    }
    ...
    private void undo() {
        ...
        redoStack.push(new HistoryState(editorBox.getValue(), getCursorSafely(), getScrollAmountSafely()));
        HistoryState state = undoStack.pop();

        editorBox.setValue(state.text());
        setCursorAbsolute(state.cursor());
        setScrollAmountSafely(state.scroll());
    ...
    private boolean handleSmartEnter() {
        ...
        if (isAfterOpenBrace && isBeforeCloseBrace) {
            ...
            isRestoringHistory = true;
            double currentScroll = getScrollAmountSafely();
            editorBox.setValue(newText);
            setCursorAbsolute(cursor + insert.length());
            setScrollAmountSafely(currentScroll);
            scrollToCursorCentered();
            isRestoringHistory = false;

            lastText = newText;
            lastCursor = getCursorSafely();
            lastScroll = getScrollAmountSafely();

            this.isDirty = !newText.equals(this.originalScriptText);
            updateLineMap();
        } else {
            insertTextSafely(insert);
        }
        return true;
    }
    ...
    public void mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (mouseX >= x + GUTTER_WIDTH && mouseX <= x + width && mouseY >= y && mouseY <= y + height) {
            double relativeY = mouseY - (y + 8) + getScrollAmountSafely();
            if (relativeY >= 0) {
    ...
    public void renderUnderlay(GuiGraphics g) {
        ...
        int fontHeight = 9;
        double scroll = getScrollAmountSafely();
        ...
        for (int i = 0; i <= maxVisibleLines; i++) {
            ...
            MultilineTextField.StringView lineView = getLineViewSafely(visualIdx);
            if (lineView != null) {
                ...
                int actualCursorLine = getLineAtCursorSafely();
                if (this.editorBox.isFocused() && visualIdx == actualCursorLine && getCursorSafely() == getSelectCursorSafely()) {
                    String textBeforeCursor = rawText.substring(bBegin, getCursorSafely());
                    ...
                    int cursorColor = FastColor.ARGB32.color(alpha, 220, 220, 220);
            ...
            if (visualIdx < visualLinesCache.size()) {
                g.drawString(this.font, visualLinesCache.get(visualIdx), textStartX, drawY, 0xFFFFFFFF, false);
            }
            ...
            g.drawString(this.font, numStr, innerX + GUTTER_WIDTH - 6 - numW, drawY, numColor, false);
        }
    ...
    private void renderAutoComplete(GuiGraphics g, int ax, int ay) {
        int itemH = 12;
        int maxItems = 6;
        ...
        g.fill(ax, ay, ax + menuW, ay + 1, 0xFF454545);
        g.fill(ax, ay + menuH - 1, ax + menuW, ay + menuH, 0xFF454545);
        g.fill(ax, ay + 1, ax + 1, ay + menuH - 1, 0xFF454545);
        g.fill(ax + menuW - 1, ay + 1, ax + menuW, ay + menuH - 1, 0xFF454545);
        ...
        for (int i = 0; i < visibleItems; i++) {
            ...
            int prefixLen = getCursorSafely() - wordStartIdx;
            if (prefixLen > 0 && prefixLen <= s.length()) {
                String prefix = s.substring(0, prefixLen);
                String rest = s.substring(prefixLen);
                int px = ax + 6;
                g.drawString(font, prefix, px, itemY + 2, 0xFF00A2FF, false);
                g.drawString(font, rest, px + font.width(prefix), itemY + 2, 0xFFCCCCCC, false);
            } else {
                g.drawString(font, s, ax + 6, itemY + 2, 0xFFCCCCCC, false);
            }
        }
    }
