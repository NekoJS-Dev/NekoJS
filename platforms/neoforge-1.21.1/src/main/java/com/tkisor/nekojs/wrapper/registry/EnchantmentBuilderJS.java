 ...
 * 解析 item 注册表为 {@link HolderSet} 并经 {@link Enchantment.Builder#build(ResourceLocation)} 组装。
 *
...
public class EnchantmentBuilderJS {
    ...
    public EnchantmentBuilderJS(ResourceLocation location) {
     ...
     * {@code HolderLookup}）以解析物品标签为 {@link HolderSet}。在 ENCHANTMENT 注册阶段
     * 标签尚未绑定，{@code getOrCreateTag} 返回的是惰性绑定的 {@code HolderSet.Named}，
     * 仅在附魔被实际查询时才解析，故此构造期安全。
     */
    ...
    private HolderSet<Item> resolveItems(Registry<Item> itemRegistry) {
        ...
        ResourceLocation tagId = ResourceLocation.tryParse(supportedItemsTag);
        if (tagId == null) {
            return HolderSet.empty();
        }
        return itemRegistry.getOrCreateTag(TagKey.create(Registries.ITEM, tagId));
    }
