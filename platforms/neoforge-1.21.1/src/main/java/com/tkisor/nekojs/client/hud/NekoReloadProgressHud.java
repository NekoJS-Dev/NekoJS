...
/**
 * Reload progress HUD (feature 8e), 1.21.1 mirror of the 26-shared
 * {@code com.tkisor.nekojs.client.hud.NekoReloadProgressHud}: a small semi-transparent panel
 * at the top-center of the screen showing script (re)load progress — a green progress bar
 * plus a {@code "<type> — <message> (NN%)"} label. Subscribes itself to
 * {@code NeoForge.EVENT_BUS} from {@link #install()}; the subscription lives here (not in
 * {@code ClientEvents}) so the feature stays self-contained.
 *
...
public final class NekoReloadProgressHud {
    ...
    static void render(GuiGraphics graphics, Minecraft minecraft) {
        ...
        graphics.drawString(minecraft.font, label, left + PADDING, top + 7, COLOR_TEXT, true);
    }
}
