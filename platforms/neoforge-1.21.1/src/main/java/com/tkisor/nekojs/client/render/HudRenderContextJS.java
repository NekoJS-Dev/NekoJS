...
/**
 * {@code ClientEvents.hudRender} 回调上下文（1.21.1 的 {@link GuiGraphics}）：
 * 携带 partialTick 与缩放后屏幕尺寸，并提供常用绘制助手（移植自 Katton 的
 * HUD draw helpers，命名对齐 {@code PainterJS}；{@code drawText}/{@code fillRect}/
 * {@code drawTexture} 为任务规格别名）。
 *
...
public class HudRenderContextJS {
    ...
    /** 原始图形对象（1.21.1 {@link GuiGraphics}），供脚本直接调用未包装的能力。 */
