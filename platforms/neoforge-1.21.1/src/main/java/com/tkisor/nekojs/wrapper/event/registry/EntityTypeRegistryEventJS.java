...
public class EntityTypeRegistryEventJS {
    private static final Map<ResourceLocation, EntityType<? extends LivingEntity>> REGISTERED_ENTITY_TYPES = new HashMap<>();
    private static final Map<ResourceLocation, AttributeSupplier> PENDING_ATTRIBUTES = new HashMap<>();
    /** 请求 spawn egg 的 builder（ENTITY_TYPE 分支填充，ITEM 分支消费注册 {@code <id>_spawn_egg}）。 */
    public static final Map<ResourceLocation, EntityTypeBuilderJS> PENDING_SPAWN_EGGS = new HashMap<>();
    /** 已注册的 spawn egg item id（跨 reload 保留，客户端生成模型用）。 */
    private static final Set<ResourceLocation> REGISTERED_SPAWN_EGGS = new HashSet<>();
        ...
        for (EntityTypeBuilderJS builder : builders) {
            ...
            rawEvent.register(Registries.ENTITY_TYPE, location, () -> {
                ...
                if (builder.isSpawnEggRequested()) {
                    ...
                    REGISTERED_SPAWN_EGGS.add(ResourceLocation.fromNamespaceAndPath(
    ...
    public static EntityType<? extends LivingEntity> getEntityType(ResourceLocation location) {
        return REGISTERED_ENTITY_TYPES.get(location);
    }

    public static Set<ResourceLocation> registeredSpawnEggs() {
