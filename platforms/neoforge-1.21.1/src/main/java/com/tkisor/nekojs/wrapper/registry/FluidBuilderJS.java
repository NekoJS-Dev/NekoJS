 ...
 *
 * <p>1.21.1 的流体纹理通过 {@code IClientFluidTypeExtensions}（客户端扩展）设置，
 * 本构建器不暴露纹理 API（与 26.x 对齐，留给资源包 / 后续扩展）；流体类型通过
 * {@link FluidType} 注册到 {@code NeoForgeRegistries.FLUID_TYPES}。实现为简单流体：
 * 单一源流体、无流动、无流体方块（{@code createLegacyBlock} 返回空气）。
 */
