 ...
 * 1.21.1 流体纹理走 {@code IClientFluidTypeExtensions} + 资源包，不提供 still/flowing 纹理 API。
 */
public class FluidBuilderJS {
    ...
    public FluidBuilderJS(ResourceLocation location) {
    ...
    public ResourceLocation getFlowingLocation() {
        return ResourceLocation.fromNamespaceAndPath(location.getNamespace(), "flowing_" + location.getPath());
    }

    /** 桶物品 id：{@code <path>_bucket}。 */
    public ResourceLocation getBucketLocation() {
        return ResourceLocation.fromNamespaceAndPath(location.getNamespace(), location.getPath() + "_bucket");
    }
     ...
     * 1.21.1 的 Item.Properties 无 setId()（26.x 才有），直接 new。
     */
